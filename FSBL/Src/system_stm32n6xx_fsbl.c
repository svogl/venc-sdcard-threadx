...
extern uint32_t __region_DTCM_start__;
extern uint32_t __region_DTCM_end__;
#define INTVECT_START ((uint32_t) &__vector_table)
#define DTCM_START    ((uint32_t *) &__region_DTCM_start__)
#define DTCM_END      ((uint32_t *) &__region_DTCM_end__)
...
#define DTCM_START    ((uint32_t *) &_sDTCM)
#define DTCM_END      ((uint32_t *) &_eDTCM)
#endif
...
void SystemInit(void)
{
  ...
  for(uint32_t * ptr = DTCM_START; ptr < DTCM_END; ptr++){
    if((ptr >= ((uint32_t *) __get_MSP())) && (ptr < DTCM_END)){
      continue;
    }
    *ptr = 0xa5a5a5a5;
  }
